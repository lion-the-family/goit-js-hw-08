...
// Событие Делегирование
container.addEventListener("click", (event) => {
  event.preventDefault(); //Отключаем дефолтные
  ...
  console.log(largeImageUrl); // Выводим ссылку на большое изображение в консоль
});

// Библиотека basicLightbox  И Модальное окно
container.addEventListener("click", (event) => {
  const clickedImage = event.target;
  if (!clickedImage.classList.contains("gallery-image")) {
    return;
